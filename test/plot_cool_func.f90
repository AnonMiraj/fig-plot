program chess_checker
    ...
    do i = 1, n_size
        ...
        x_data(i) = sin(3.0 * (i-1) * 2.0 * pi / (n_size - 1))
        y_data(i) = sin(4.0 * (i-1) * 2.0 * pi / (n_size - 1))
    end do

    call pp%plot(x_data, y_data, n_size)

    do i = 1, n_size
        x_data(i) = (i-1) * cos(2.0 * pi * (i-1) / (n_size - 1)) / (n_size - 1)
        y_data(i) = (i-1) * sin(2.0 * pi * (i-1) / (n_size - 1)) / (n_size - 1)
    end do
